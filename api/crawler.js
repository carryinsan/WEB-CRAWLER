 ...
 * 
 * Search snippets are discovery evidence. If full fetch fails, 
 * the snippet is used as a fallback to ensure ALL discovered candidates 
 * are returned to the user, ranked by relevance.
 */
...
const VERSION = 'arix-crawler-1.11.0';
...
const USER_AGENT = 'Mozilla/5.0 (compatible; ArixAI-LiveSearch/1.11.0; +https://lexis-ai-chatini.vercel.app/)';
...
function normalizedKey(url) {
  try {
    ...
    for (const k of [...u.searchParams.keys()]) {
      if (/^(utm_gclid$|fbclid$|msclkid$|ref$|referrer$|cmpid$|src$)/i.test(k)) u.searchParams.delete(k);
    }
...
function unwrap(raw, base) {
  ...
  for (let depth = 0; depth < 4; depth++) {
    ...
    if (!next && /^google\./i.test(host) && /^\/url$/i.test(u.pathname)) {
      next = u.searchParams.get('url') || u.searchParams.get('q');
    }
    if (!next && (host === 'duckduckgo.com' || host === 'html.duckduckgo.com') && /^\/l\/?$/i.test(u.pathname)) {
...
function providerRequests(queries, plan, count) {
  ...
  const add = (provider, q, type, url) => {
    ...
    seen.add(url); 
    final.push({ provider, query: q, type, url });
  };
...
async function groqRerank(query, results, deadline) {
  ...
  try {
    const payload = results.slice(0, 30).map((r, i) => ({
      id: i, title: truncate(r.title, 180), domain: normalizeHost(r.url), 
      type: r.type, relevanceScore: r.relevanceScore, snippet: truncate(r.snippet, 240)
    }));
...
function diversifyFinal(results, count) {
  ...
  while (selected.length < count && pool.length) {
    ...
    for (const c of chosen.relevance?.matchedConcepts || []) topicSeen.set(c, (topicSeen.get(c) || 0) + 1);
  }
  return selected;
}
...
async function performSearch(input, started, logger) {
  ...
  logger.add('query-analyzed', `Precision query analysis completed.`, { type: plan.type, concepts: plan.concepts.map(x => x.id), dateIntent: plan.dateIntent.kind });
  ...
  logger.add('discovery-complete', `Discovery produced ${discovered.length} unique candidates.`, { successfulProviders: discoveryOk, sourceCandidates: discovered.length });
  ...
  logger.add('ranking-complete', `Prepared ${acquisitionPool.length} candidates for page acquisition.`);
  ...
  if (!verifyRequested) {
    const metadataOnly = diversifyFinal(ranked.slice(0, count).map(r => ({
      ...
      verified: false,
      pageContent: r.snippet,
      extractedText: r.snippet,
      contentAvailable: false
    })), count);
    const final = metadataOnly.map((r, i) => formatResult(r, i, plan));
  ...
  try {
    const checked = await enrichCandidates(contentCandidates, plan, { count, requireRealContent: true, budgetMs: algorithmBudget });
    enriched = Array.isArray(checked?.results) ? checked.results : [];
  } catch (error) {
    logger.add('content-failed', 'The page-content checker failed safely.', { error: error?.message || 'CONTENT_CHECK_FAILED' });
  }

  // Map the verified/enriched pages so we can use them
  const enrichedMap = new Map(enriched.map(r => [normalizedKey(r.url), r]));

  // **CRITICAL FIX**: Instead of dropping failed fetches, we merge everything back together. 
  // If we fetched the content, we use it. If not, we fall back to metadata and score the snippet.
  const mergedCandidates = contentCandidates.map(c => {
    const k = normalizedKey(c.url);
    if (enrichedMap.has(k)) {
      return enrichedMap.get(k); // Full verified page content
    }
    
    // Create a fallback object with the snippet acting as the content
    const fallback = { ...c, pageContent: c.snippet, extractedText: c.snippet };
    
    // Compare this fallback to the query so it can be ranked against the verified pages
    const rel = comparePageToQuery(plan.query, fallback);
    
    return {
      ...fallback,
      relevance: rel,
      relevanceScore: Number(rel.score || 0),
      relevanceBand: rel.band || 'weak-match',
      contentAvailable: false,
  ...
  logger.add('content-complete', `Page fetch + query comparison completed on all ${mergedCandidates.length} sources.`, {
    candidates: contentCandidates.length,
    fullyVerified: enrichedMap.size,
    totalMerged: mergedCandidates.length
  });

  // Rank ALL of them based on the unified score
  const finalPool = [...new Map(mergedCandidates.map(r => [normalizedKey(r.url), r])).values()]
    .slice(0, OUTPUT_MAX_SOURCES)
    .map(r => ({ ...r, _score: Number(r.relevanceScore || 0) }))
    .sort((a, b) => Number(b.relevanceScore || 0) - Number(a.relevanceScore || 0));

  const chosen = finalPool; // Return ALL discovered sources that passed through the ranking
  ...
  const final = chosen.map((r, i) => formatResult({ ...r, commonCrawl: ccMap.get(normalizedKey(r.url)) || null }, i, plan));
...
function formatResult(r, i, plan) {
  const content = String(r.pageContent || r.extractedText || r.snippet || '').trim();
  const relevanceScore = Math.round(Number(r.relevanceScore ?? r._score ?? 0));
  ...
  return {
    ...
    pageContent: content,
    contentAvailable: Boolean(r.contentAvailable),
    contentStatus: r.contentStatus || (r.contentAvailable ? 'full' : 'metadata-fallback'),
    contentMethod: r.contentMethod || r.validatedBy || (r.contentAvailable ? 'algorithm' : 'search-snippet'),
    contentLength: content.length,
    contentConfidence: Number(r.contentConfidence ?? (r.contentAvailable ? 0.9 : 0)),
    contentSourceUrl: r.contentSourceUrl || r.url,
    contentTargetMatched: Boolean(r.contentTargetMatched ?? r.contentAvailable),
    contentTitleSimilarity: Number(r.contentTitleSimilarity ?? rel.titleCoverage ?? 0).toFixed(3),
    contentConceptCoverage: Number(r.contentConceptCoverage ?? rel.conceptCoverage ?? 0).toFixed(3),
    contentFormat: 'plain_text',
    contentRole: r.contentAvailable ? 'publisher_page_content' : 'search_metadata_only',
    contentForAI: r.contentAvailable ? `SOURCE_URL: ${r.contentSourceUrl || r.url}\nTITLE: ${truncate(r.title || '', 300)}\nCONTENT_STATUS: ${r.contentStatus || 'full'}\n\n${content}` : null,
...
function buildResponse({ query, count, mode, requestedType, plan, preciseQueries, providerStats, final, logger, started, verifyRequested, requireRealContent, useCc, deep, aiRequested }) {
  ...
  const result = {
    ...
    sourceCountMode: 'all-discovered-candidates',
    ...
    quality: {
      ...
      contentGuarantee: 'All discovered sources are returned. Successfully verified pages include their full pageContent. Unverified pages fall back to metadata snippets to ensure candidates are not lost.',
    },
    searchPlan: {
  ...
  if (requireRealContent && validationCount < count) {
    result.warnings.push(`${validationCount} fully verified pages were fetched; ${count} were requested. All ${final.length} discovered sources are included below using snippet fallbacks where full fetch failed.`);
  }
  if (!final.length) {
    result.warnings.push('No sources satisfied the request. No page content was found.');
  }
  if (result.latencyMs > SEARCH_BUDGET_MS) {
    result.warnings.push('The outer platform/runtime may have added latency beyond the crawler work budget.');
  }
  return result;
}
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      Promise.resolve().then(() => performSearch(input, started, logger)).then(result => {
        ...
        entries.forEach(([k, v], i) => send(`"${k}":${JSON.stringify(v)}${i === entries.length - 1 ? '' : ',\n'}`));
        send('}');
