...
function queryPlan(query, options = {}) {
  ...
  return {
    ...
    aiTargets:[], aiQueries:[], aiInformationNeed:'',
...
function groqApiKeys(){
  const names=['GROQ_API_KEY',...Array.from({length:9},(_,i)=>`GROQ_API_KEY_${i+2}`)];
  return unique(names.map(env).filter(Boolean));
}
...
function normalizePlannerPlan(raw, basePlan){
  ...
  let qCounter=0;
  for(let i=0;i<rawTargets.length && targets.length<MAX_AI_TARGETS;i++){
    const t=rawTargets[i]||{};
    const objective=truncate(String(t.objective||t.goal||'').replace(/\s+/g,' ').trim(),500);
    if(objective.length<8)continue;
    const id=String(t.id||`T${targets.length+1}`).trim().replace(/[^A-Za-z0-9_-]/g,'').slice(0,24)||`T${targets.length+1}`;
    if(targetSeen.has(id))continue;
    targetSeen.add(id);
    const anchors=unique((Array.isArray(t.anchors)?t.anchors:[]).map(x=>normalizeText(x)).filter(x=>x.length>=2)).slice(0,10);
    const requiredEvidence=unique((Array.isArray(t.requiredEvidence)?t.requiredEvidence:[]).map(x=>truncate(String(x||''),180)).filter(Boolean)).slice(0,8);
    const timeConstraint=truncate(String(t.timeConstraint||t.time||(basePlan.flags.live?'current':'' )).trim(),100);
    const geography=truncate(String(t.geography||'').trim(),100);
    const sourceType=truncate(String(t.sourceType||t.sourceClass||'general').trim(),80).toLowerCase();
    const priority=Math.max(1,Math.min(10,Number(t.priority)||targets.length+1));
    const queries=Array.isArray(t.queries)?t.queries:[];
    const normalizedQueries=[];
    const seenQ=new Set();
    for(let j=0;j<queries.length && normalizedQueries.length<MAX_QUERIES_PER_TARGET;j++){
      const qv=queries[j];
      const query=truncate(String(typeof qv==='string'?qv:(qv?.query||qv?.q||'')).replace(/\s+/g,' ').trim(),460);
      if(query.length<3)continue;
      const key=normalizeText(query);
      if(seenQ.has(key))continue;
      seenQ.add(key);
      const purpose=truncate(String(typeof qv==='string'?'':(qv?.purpose||qv?.goal||'')).replace(/\s+/g,' ').trim(),260) || objective;
      const strategy=truncate(String(typeof qv==='string'?'general':(qv?.strategy||qv?.style||'general')).trim(),60).toLowerCase();
      const queryId=`Q${++qCounter}`;
      const pq={id:queryId,targetId:id,query,purpose,strategy,priority,objective,sourceType,anchors,timeConstraint,geography};
      normalizedQueries.push(pq); allQueries.push(pq);
    }
    if(!normalizedQueries.length)continue;
    targets.push({id,objective,priority,sourceType,anchors,requiredEvidence,timeConstraint,geography,queries:normalizedQueries});
  }
  // If the model under-produced query missions, fail rather than silently using
  // a different/non-AI query strategy. The Groq planner is compulsory.
  if(allQueries.length<MIN_AI_QUERIES)throw new Error('AI_PLANNER_TOO_FEW_QUERIES');
  const limitedQueries=allQueries.slice(0,MAX_AI_QUERIES);
  return {
    informationNeed:truncate(String(source.informationNeed||source.researchScope||source.summary||'').replace(/\s+/g,' ').trim(),900) || basePlan.query,
    searchType:truncate(String(source.searchType||source.type||basePlan.type).trim(),40).toLowerCase() || basePlan.type,
    targets,
    queries:limitedQueries
  };
}

async function createAiSearchPlan(query, basePlan, deadline, logger){
  const keys=groqApiKeys();
  if(!keys.length)throw new Error('GROQ_API_KEY_REQUIRED');
  const started=Date.now();
  const complexity=basePlan.longQuery || basePlan.terms.length>12 || basePlan.flags.official || basePlan.flags.academic ? 'medium' : 'low';
  const prompt=`You are the compulsory search-strategy engine for ArixAI. Your ONLY job is to convert ONE user search request into a hyper-precise web retrieval plan. Do NOT answer the user. Do NOT invent facts. Return ONLY valid JSON.

USER SEARCH REQUEST:
${query}

DETERMINISTIC SIGNALS:
${JSON.stringify({type:basePlan.type,terms:basePlan.terms.slice(0,24),coreTerms:basePlan.coreTerms,anchors:basePlan.anchors,quotedPhrases:basePlan.quotedPhrases,live:basePlan.flags.live,history:basePlan.flags.history,official:basePlan.flags.official,academic:basePlan.flags.academic})}

Your plan must obey all of these rules:
1. Decompose the request into small, independently searchable information TARGETS. A target answers one concrete factual sub-question or retrieves one evidence class. Complex requests need several targets; simple requests can use fewer.
2. Do NOT create near-duplicate targets or near-duplicate queries. Every query must have a distinct retrieval JOB.
3. Preserve exact entity names, product/model names, law/act/rule names, section numbers, scheme names, identifiers, places, dates, years and quoted phrases. Never replace exact entities with broad synonyms.
4. Use controlled terminology expansion only when vocabulary mismatch is likely. Prefer the terminology an authoritative source is likely to use.
5. For every target create 1-3 compact queries. Most queries should be short, concrete and information-dense, normally about 3-14 meaningful terms. Avoid giant natural-language questions.
6. Vary the query strategy when useful: direct_fact, exact_phrase, authoritative_source, document_pdf, official_domain, terminology, freshness, corroboration, contradiction_check. Do not use all styles mechanically.
7. Match source strategy to topic: laws/regulations -> legislation/notifications/rules/orders; government schemes -> official scheme pages/guidelines/portals; software/APIs -> official docs/pricing/changelog; news -> primary announcements plus independent reporting; academic -> papers/publishers; products -> manufacturer specifications plus independent testing.
8. Use site: ONLY when the authority/domain is strongly justified by the request or universally obvious. Never invent a domain. A site restriction must not erase useful non-site discovery.
9. For current/latest questions, encode freshness using dates/year/current terms. Do not leave time vague.
10. Include targeted corroboration when the answer could be disputed, outdated, or numerically sensitive. Include contradiction-check retrieval when sources may disagree.
11. Search missions must maximize precision and evidence quality, NOT URL count. Do not broaden queries just to get more results.
12. At most 10 targets and 18 total queries. Prefer fewer strong queries over many broad ones.
13. Return this exact JSON shape: {"informationNeed":"...","searchType":"web|news|gov|doc|video|academic|software|product","targets":[{"id":"T1","objective":"...","priority":1,"sourceType":"official_primary|official_guidance|document_pdf|reputable_reporting|technical_docs|academic|manufacturer|general","anchors":["..."],"requiredEvidence":["..."],"timeConstraint":"...","geography":"...","queries":[{"query":"...","purpose":"...","strategy":"direct_fact|exact_phrase|authoritative_source|document_pdf|official_domain|terminology|freshness|corroboration|contradiction_check"}]}]}
14. Never provide an answer, summary of the topic, fake sources, fake URLs, or simulated search results.`;

  const startIndex=hashString(query)%keys.length;
  let lastError=null;
  const attempts=Math.min(keys.length,3);
  for(let a=0;a<attempts;a++){
    const key=keys[(startIndex+a)%keys.length];
    const timeout=Math.min(AI_PLANNER_TIMEOUT_MS,Math.max(650,left(deadline)-120));
    ...
    try{
      ...
      if(left(deadline)<550)break;
    }finally{clearTimeout(timer)}
  }
  throw lastError||new Error('AI_PLANNER_FAILED');
}

function buildQueries(query, plan, deep = false) {
  const planned=Array.isArray(plan.aiQueries)?plan.aiQueries:[];
  const max=deep?Math.min(MAX_AI_QUERIES,14):Math.min(MAX_AI_QUERIES,12);
  if(!planned.length)return [];
  // Interleave target missions so the request budget cannot be monopolized by
  // three query variants from the first target. Coverage of distinct targets wins.
  const byTarget=new Map();
  for(const q of planned){
    const id=q.targetId||'__ungrouped';
    if(!byTarget.has(id))byTarget.set(id,[]);
    byTarget.get(id).push(q);
  }
  const groups=[...byTarget.values()];
  const out=[];
  for(let round=0;round<MAX_QUERIES_PER_TARGET && out.length<max;round++){
...
function buildTargetIndex(plan){
  const map=new Map();
  for(const t of (plan.aiTargets||[])) map.set(t.id,t);
  return map;
}
...
function providerRequests(queries, plan, deep) {
  ...
  const add=(provider,qobj,type,url)=>{
    if(out.length>=MAX_ENGINE_REQUESTS || !url || seen.has(url))return;
    seen.add(url);
    out.push({
      provider, query:String(qobj.query||''), type, url,
      queryVariant:Number(qobj.index||0), queryId:qobj.id||null, targetId:qobj.targetId||null,
      targetGoal:qobj.objective||'', targetPriority:Number(qobj.priority||5),
      sourceType:qobj.sourceType||'general', queryPurpose:qobj.purpose||'', queryStrategy:qobj.strategy||'general',
      plannedQuery:qobj.query||'', anchors:qobj.anchors||[], timeConstraint:qobj.timeConstraint||'', geography:qobj.geography||''
    });
  };
  const qs=queries.map((q,index)=>({...q,index}));
  const type=plan.type==='news'?'news':'web';

  // Every AI-generated target gets real retrieval coverage first. Additional engines
  // are layered on the highest-value missions without duplicating the same query everywhere.
  for(const q of qs){
    const e=encodeURIComponent(q.query);
    add('bing',q,type,`https://www.bing.com/search?q=${e}&count=10&first=0&form=QBLH&setlang=en-IN&cc=in`);
    add('google',q,type,`https://www.google.com/search?gbv=1&q=${e}&num=20&hl=en&gl=in&filter=0`);
    if(out.length>=MAX_ENGINE_REQUESTS)break;
  }

  for(let i=0;i<Math.min(4,qs.length) && out.length<MAX_ENGINE_REQUESTS;i++){
    const q=qs[i],e=encodeURIComponent(q.query);
    add('duckduckgo',q,type,`https://html.duckduckgo.com/html/?q=${e}&kl=in-en`);
    if(out.length>=MAX_ENGINE_REQUESTS)break;
  }
  for(let i=0;i<Math.min(3,qs.length) && out.length<MAX_ENGINE_REQUESTS;i++){
    const q=qs[i],e=encodeURIComponent(q.query);
    add('yahoo',q,type,`https://search.yahoo.com/search?p=${e}&fr=yfp-t`);
  }
  for(let i=0;i<Math.min(2,qs.length) && out.length<MAX_ENGINE_REQUESTS;i++){
    const q=qs[i],e=encodeURIComponent(q.query);
    add('mojeek',q,type,`https://www.mojeek.com/search?q=${e}&lb=EN&lbb=100&rb=IN&rbb=10&fmt=html`);
    add('brave',q,type,`https://search.brave.com/search?q=${e}&source=web`);
  }
  if(plan.flags.live || plan.type==='news'){
    for(let i=0;i<Math.min(2,qs.length) && out.length<MAX_ENGINE_REQUESTS;i++){
      const q=qs[i]; add('google-news',q,'news',`https://news.google.com/rss/search?q=${encodeURIComponent(q.query)}&hl=en-IN&gl=IN&ceid=IN:en`);
    }
  }
  if(plan.type==='video' || plan.flags.explicitVideo){
    for(let i=0;i<Math.min(2,qs.length) && out.length<MAX_ENGINE_REQUESTS;i++){
      const q=qs[i]; add('youtube',q,'video',`https://www.youtube.com/results?search_query=${encodeURIComponent(q.query)}&hl=en`);
    }
  }
  if(plan.type==='doc' || plan.flags.explicitDoc){
    for(let i=0;i<Math.min(3,qs.length) && out.length<MAX_ENGINE_REQUESTS;i++){
      const q=qs[i]; add('google-doc',q,'doc',`https://www.google.com/search?gbv=1&q=${encodeURIComponent(`${q.query} filetype:pdf`)}&num=20&hl=en&gl=in&filter=0`);
    }
  }
  if(plan.type==='gov' || plan.flags.explicitGov || plan.flags.official){
    for(let i=0;i<Math.min(3,qs.length) && out.length<MAX_ENGINE_REQUESTS;i++){
      const q=qs[i]; add('google-gov',q,'gov',`https://www.google.com/search?gbv=1&q=${encodeURIComponent(`${q.query} site:gov.in`)}&num=20&hl=en&gl=in&filter=0`);
    }
  }
  return out;
}
...
async function fetchWithTimeout(url, timeout, deadline, headers = {}) {
  ...
  try{
    return await fetch(url,{method:'GET',redirect:'follow',cache:'no-store',signal:controller.signal,headers:{'user-agent':USER_AGENT,'accept':'text/html,application/xhtml+xml,application/xml,application/rss+xml,text/plain;q=0.9,application/pdf;q=0.8,*/*;q=0.1','accept-language':'en-IN,en;q=0.9',...headers}});
  } finally { clearTimeout(timer); }
}
...
async function discoverOne(req,deadline){
  try{
    ...
    results=results.map(r=>({...r,targetId:req.targetId||null,targetIds:unique([...(r.targetIds||[]),req.targetId].filter(Boolean)),targetGoal:req.targetGoal||'',targetPriority:Number(req.targetPriority||5),sourceType:req.sourceType||'general',queryId:req.queryId||null,queryPurpose:req.queryPurpose||'',queryStrategy:req.queryStrategy||'general',plannedQuery:req.plannedQuery||req.query,anchors:req.anchors||[],timeConstraint:req.timeConstraint||'',geography:req.geography||''}));
    if(!results.length && !['google-news','bing-rss','youtube'].includes(req.provider)) {
      results = extractAnchors(body, req.url, req.provider, req, 40).map(r=>({...r,targetId:req.targetId||null,targetIds:unique([...(r.targetIds||[]),req.targetId].filter(Boolean)),targetGoal:req.targetGoal||'',targetPriority:Number(req.targetPriority||5),sourceType:req.sourceType||'general',queryId:req.queryId||null,queryPurpose:req.queryPurpose||'',queryStrategy:req.queryStrategy||'general',plannedQuery:req.plannedQuery||req.query,anchors:req.anchors||[],timeConstraint:req.timeConstraint||'',geography:req.geography||''}));
    }
...
function dedupe(list){
  ...
  for(const raw of list||[]){
    ...
    if(!old){
      map.set(key,{...raw,url,targetIds:unique([...(raw.targetIds||[]),raw.targetId].filter(Boolean)),providers:unique(raw.providers||[raw.source])});
      continue;
    }
    const providers=new Set([...(old.providers||[old.source]),...(raw.providers||[raw.source])].filter(Boolean));
    const targetIds=unique([...(old.targetIds||[]),...(raw.targetIds||[]),old.targetId,raw.targetId].filter(Boolean));
    const candidates=[old,raw].sort((a,b)=>{
      const ap=Number(a.targetPriority||5),bp=Number(b.targetPriority||5); if(ap!==bp)return ap-bp;
      const ar=Number(a.providerRank||99),br=Number(b.providerRank||99); return ar-br;
    });
    const best=candidates[0];
    const merged={...old,...raw,url,providers:[...providers],targetIds,source:providers.size>1?'multi-source':best.source||old.source};
    ...
    if(!raw.title&&old.title)merged.title=old.title;
    if(old.queryPurpose && raw.queryPurpose && old.queryPurpose!==raw.queryPurpose){
      merged.queryPurposes=unique([...(old.queryPurposes||[old.queryPurpose]),raw.queryPurpose]).slice(0,6);
    }
    if(old.plannedQuery && raw.plannedQuery && old.plannedQuery!==raw.plannedQuery){
      merged.plannedQueries=unique([...(old.plannedQueries||[old.plannedQuery]),raw.plannedQuery]).slice(0,8);
    }
    map.set(key,merged);
  }
...
function targetTermsForItem(item,plan){
  const direct=unique([
    ...(item.anchors||[]),
    ...contentTokens(item.targetGoal||''),
    ...contentTokens(item.plannedQuery||''),
    ...contentTokens(item.queryPurpose||'')
  ]);
  if(direct.length>=2)return direct.slice(0,24);
  const targetIds=unique(item.targetIds||[item.targetId]);
  const byId=new Map((plan.aiTargets||[]).map(t=>[t.id,t]));
  const out=[];
  for(const id of targetIds){
    const t=byId.get(id); if(!t)continue;
    out.push(...(t.anchors||[]),...contentTokens(t.objective||''));
  }
  return unique(out).slice(0,24);
}
function targetMatch(item,plan){
  const terms=targetTermsForItem(item,plan);
  if(!terms.length)return {score:0,titleCoverage:0,snippetCoverage:0,matched:[],targetId:item.targetId||null,objective:item.targetGoal||''};
  const title=weightedCoverage(item.title||'',terms);
  const snippet=weightedCoverage(`${item.snippet||''} ${item.pageProbeText||''}`,terms);
  const exact=phraseScore(item.plannedQuery||item.queryPurpose||'',`${item.title||''} ${item.snippet||''} ${item.pageProbeText||''}`);
  let score=title.coverage*58+snippet.coverage*26+exact*12;
  const dq=domainQuality(item.url||''); score+=dq*4;
  if(item.verified)score+=3;
  return {
    score:clamp(Math.round(score*100)/100,0,100),
    titleCoverage:Number(title.coverage.toFixed(3)),
    snippetCoverage:Number(snippet.coverage.toFixed(3)),
    matched:unique([...title.matched,...snippet.matched]),
    targetId:item.targetId||null,
    objective:item.targetGoal||'',
    priority:Number(item.targetPriority||5)
  };
}

function sourceClassBonus(item,plan){
  const st=String(item.sourceType||'').toLowerCase();
  const h=normalizeHost(item.url||'');
  if(st==='official_primary')return isGov(item.url)||isTrusted(item.url)?6:1;
  if(st==='official_guidance')return isGov(item.url)||isTrusted(item.url)?5:1;
  if(st==='document_pdf')return isDoc(item.url)?4:0;
  if(st==='technical_docs')return /(?:docs?|developer|learn|reference|api|documentation)/i.test(h)||isTrusted(item.url)?3:0;
  if(st==='academic')return /(?:\.edu$|\.ac\.|arxiv\.|doi\.|nature\.|science\.)/i.test(h)?4:1;
  if(st==='manufacturer')return /(?:apple|microsoft|google|samsung|sony|intel|amd|nvidia|qualcomm|lenovo|dell|hp)\./i.test(h)?4:1;
  if(st==='reputable_reporting')return /(?:reuters|apnews|bbc|theguardian|nytimes|economist)\./i.test(h)?4:1;
  return 0;
}
...
function rankOne(item,plan){
  ...
  const queryForPhrase=item.plannedQuery||plan.quotedPhrases?.[0]||plan.coreQuery||plan.query;
  const phrase=phraseScore(queryForPhrase,`${title} ${snippet} ${item.pageProbeText||''}`);
  const target=targetMatch(item,plan);
  ...
  const priorityBonus=Math.max(0,Math.min(5,(11-Number(item.targetPriority||5))*.55));

  let score=0;
  score += t.coverage*24*specificity;
  score += s.coverage*11;
  score += Math.min(.75,u.coverage)*3;
  score += phrase*7;
  score += probe.coverage*4;
  score += target.score*0.44;
  score += consensus*4;
  score += qRank*3;
  score += dq*5;
  score += sourceClassBonus(item,plan);
  score += priorityBonus;
  ...
  if(t.coverage < .16 && s.coverage < .20) score -= plan.longQuery ? 7 : 4;
  if(t.coverage < .10 && target.score < 25 && phrase < .35) score -= 4;
...
function relevanceObject(item,plan){
  const terms=(plan.coreTerms?.length ? plan.coreTerms : plan.terms) || [];
  const title=weightedCoverage(item.title,terms), body=weightedCoverage(`${item.snippet||''} ${item.pageProbeText||''}`,terms);
  const target=targetMatch(item,plan);
  const phrase=phraseScore(item.plannedQuery||plan.quotedPhrases?.[0]||plan.coreQuery||plan.query,`${item.title} ${item.snippet} ${item.pageProbeText||''}`);
  ...
  return {
    ...
    targetId:target.targetId,targetObjective:truncate(target.objective||'',400),targetPriority:target.priority||Number(item.targetPriority||5),
    targetRelevanceScore:target.score,targetTitleCoverage:target.titleCoverage,targetSnippetCoverage:target.snippetCoverage,targetMatchedConcepts:target.matched,
    queryId:item.queryId||null,plannedQuery:item.plannedQuery||null,queryPurpose:item.queryPurpose||null,queryStrategy:item.queryStrategy||null,
...
function fusionRank(results,plan){
  ...
  ranked.sort((a,b)=>{
    ...
    const aDiv=Math.min(5,ad)*1.4, bDiv=Math.min(5,bd)*1.4;
    const aa=a._selectionScore-aDiv, bb=b._selectionScore-bDiv;
    if(Math.abs(bb-aa)>.01)return bb-aa;
    const at=Number(b.relevance?.targetRelevanceScore||0)-Number(a.relevance?.targetRelevanceScore||0); if(Math.abs(at)>.01)return at;
...
function selectExactCount(ranked,count,plan){
  ...
  if(count<=1||!plan.aiTargets?.length)return pool.slice(0,count);

  // First secure target coverage when the requested result count permits it.
  // Weak target matches are not hard-filtered; they simply do not earn a coverage slot.
  const targetBest=new Map();
  for(const r of pool){
    const ids=unique([...(r.targetIds||[]),r.targetId].filter(Boolean));
    for(const id of ids){
      const existing=targetBest.get(id);
      if(!existing || Number(r.relevance?.targetRelevanceScore||0)>Number(existing.relevance?.targetRelevanceScore||0))targetBest.set(id,r);
    }
  }
  const chosen=[]; const chosenKeys=new Set();
  const targetOrder=[...(plan.aiTargets||[])].sort((a,b)=>Number(a.priority||9)-Number(b.priority||9));
  if(count>=Math.min(3,targetOrder.length)){
    for(const target of targetOrder){
      if(chosen.length>=count)break;
      const r=targetBest.get(target.id); if(!r)continue;
      if(Number(r.relevance?.targetRelevanceScore||0)<30)continue;
      const key=normalizedKey(r.url); if(chosenKeys.has(key))continue;
      chosen.push(r); chosenKeys.add(key);
    }
  }
  for(const r of pool){
    if(chosen.length>=count)break;
...
async function verifyOne(candidate,deadline){
  ...
  try{
    ...
    const pageTitle=titleFromHtml(body) || metaFromHtml(body,'og:title') || '';
...
function decorateResult(r,i,plan){
  ...
  return {
    rank:i+1,title:truncate(r.title||'Untitled',300),pageTitle:truncate(r.pageTitle||r.title||'Untitled',300),url:r.url,domain:normalizeHost(r.url),type,
    ...
    contentConceptCoverage:Number(r.relevance?.conceptCoverage||0).toFixed(3),targetTitleSimilarity:Number(r.relevance?.targetTitleCoverage||0).toFixed(3),targetSnippetSimilarity:Number(r.relevance?.targetSnippetCoverage||0).toFixed(3),contentFormat:'none',contentRole:'search_metadata_only',contentForAI:null,
    verificationMethod:r.verificationMethod||null,verificationError:r.verificationError||null,commonCrawl:r.commonCrawl||null,
    targetId:r.relevance?.targetId||r.targetId||null,targetIds:r.relevance?.targetIds||r.targetIds||[],targetObjective:r.relevance?.targetObjective||r.targetGoal||null,
    targetPriority:r.relevance?.targetPriority||r.targetPriority||null,targetRelevanceScore:Number(r.relevance?.targetRelevanceScore||0),
    plannedQuery:r.relevance?.plannedQuery||r.plannedQuery||null,queryPurpose:r.relevance?.queryPurpose||r.queryPurpose||null,queryStrategy:r.relevance?.queryStrategy||r.queryStrategy||null,queryId:r.relevance?.queryId||r.queryId||null,
    queryMatch:{planType:plan.type,liveIntent:plan.flags.live,historyIntent:plan.flags.history,officialIntent:plan.flags.official,terms:plan.terms,targetId:r.relevance?.targetId||r.targetId||null,targetObjective:r.relevance?.targetObjective||r.targetGoal||null}
  };
}

function targetCoverageSummary(results,plan){
  const hits=new Map();
  for(const r of results){
    for(const id of unique([...(r.targetIds||[]),r.targetId].filter(Boolean))){
      const existing=hits.get(id)||0; hits.set(id,existing+1);
    }
  }
  return (plan.aiTargets||[]).map(t=>({id:t.id,objective:t.objective,priority:t.priority,selectedResults:hits.get(t.id)||0,queries:t.queries.length}));
}
...
async function performSearch(input,started,logger){
  ...
  // Groq is deliberately compulsory. The legacy `ai=false` switch is ignored so that
  // connected frontends cannot accidentally bypass the precision query planner.
  const aiRequested='compulsory';
  const useCc=String(input.commonCrawl??'false').toLowerCase()==='true';

  const plan=queryPlan(query,{mode,type:requestedType});
  logger.add('query-analyzed','Built deterministic intent envelope; compulsory AI planning is next.',{type:plan.type,terms:plan.terms.slice(0,16),live:plan.flags.live,complexity:plan.longQuery?'high':'normal'});

  const aiDeadline=Math.min(deadline-700,Date.now()+AI_PLANNER_TIMEOUT_MS+250);
  const aiPlan=await createAiSearchPlan(query,plan,aiDeadline,logger);
  plan.aiTargets=aiPlan.targets;
  plan.aiQueries=aiPlan.queries;
  plan.aiInformationNeed=aiPlan.informationNeed;
  if(aiPlan.searchType && ['web','news','gov','doc','video','academic','software','product'].includes(aiPlan.searchType) && requestedType===''){
    if(aiPlan.searchType==='news'&&plan.type==='web')plan.type='news';
    if(aiPlan.searchType==='gov'&&plan.type==='web')plan.type='gov';
    if(aiPlan.searchType==='doc'&&plan.type==='web')plan.type='doc';
    if(aiPlan.searchType==='video'&&plan.type==='web')plan.type='video';
  }
  const preciseQueries=buildQueries(query,plan,deep);
  if(preciseQueries.length<MIN_AI_QUERIES)throw new Error('AI_PLANNER_TOO_FEW_QUERIES');
  logger.add('query-plan-ready',`Using ${preciseQueries.length} AI-generated precision queries across ${plan.aiTargets.length} target missions.`,{targets:plan.aiTargets.map(t=>({id:t.id,objective:t.objective,priority:t.priority,queryCount:t.queries.length})),queries:preciseQueries.map(q=>({id:q.id,targetId:q.targetId,query:q.query,strategy:q.strategy}))});

  const requests=providerRequests(preciseQueries,plan,deep);
  logger.add('discovery-start',`Launching ${requests.length} live public search requests in parallel from distinct target missions.`,{queryCount:preciseQueries.length});
  ...
  const discoveredBeforeSelection=discovered.length;
  ...
  logger.add('target-title-ranking-complete','Ranked sources using global query match plus target-specific title/snippet similarity, source quality, freshness, consensus and intent.');

  const verifyLimit=Math.min(ranked.length,deep?36:Math.max(12,Math.min(32,count*3,24+plan.aiTargets.length)));
  let verificationPerformed=0;
  if(verifyRequested && left(deadline)>650 && verifyLimit){
    logger.add('verification-start',`Running lightweight real HTTP/title checks on ${verifyLimit} leading candidates.`);
    verificationPerformed=verifyLimit;
    const verifyDeadline=Math.min(deadline-60,Date.now()+Math.max(500,left(deadline)-60));
    const checked=await mapConcurrent(ranked.slice(0,verifyLimit),Math.min(28,VERIFY_CONCURRENCY),(r)=>verifyOne(r,verifyDeadline));
    ...
    logger.add('verification-complete',`Verification finished; ${ranked.slice(0,verifyLimit).filter(r=>r.verified).length} of ${verifyLimit} candidates responded usefully.`);
  }else{
    logger.add('verification-skipped','Verification skipped because the remaining wall-clock budget was too small.');
  }

  // No second AI reranker. Groq's sole role remains query planning; final source selection is
  // deterministic, inspectable and exact-count, avoiding a second model/network bottleneck.
  const selected=selectExactCount(ranked,count,plan);
  const final=selected.filter(r=>r?.url&&!blocked(r.url)&&!isLikelySearchUrl(r.url));
  ...
  if(useCc && left(deadline)>500){
    ...
    for(const r of final)r.commonCrawl=ccMap.get(normalizedKey(r.url))||null;
  }
