/*
 * ArixAI Precision Page Algorithm — v1.1.0
 *
 * Purpose: lightweight query planning + REAL page acquisition + simple
 * post-fetch query comparison.
 *
 * Contract:
 *   1. Receive already-discovered candidate URLs.
 *   2. Fetch candidate pages concurrently.
 *   3. Extract real page content.
 *   4. Compare that content with the user's query.
 *   5. Return the best matching real-content pages.
 *
 * No snippet is promoted to pageContent. No AI reranker is required.
 * No external search-provider API is called from this module.
 */
...
function tokens(v) {
  return normalizeText(v).split(/\s+/).filter(x => x.length > 1 && !STOPWORDS.has(x));
}
...
function isTrusted(url) { const h = host(url); return TRUSTED_DOMAINS.some(x => h === x || h.endsWith(`.${x}`)) || /\.edu(?:\.|$)/i.test(h) || /\.ac\.(?:in|uk|jp|nz)$/i.test(h); }
...
function decodeHtml(v='') {
  return String(v).replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&lt;/gi,'<').replace(/&gt;/gi,'>').replace(/&quot;/gi,'"').replace(/&#39;|&apos;/gi,"'").replace(/&#x([0-9a-f]+);/gi,(_,x)=>{const n=parseInt(x,16);return Number.isFinite(n)?String.fromCodePoint(Math.min(0x10ffff,n)):_;}).replace(/&#(\d+);/g,(_,x)=>{const n=parseInt(x,10);return Number.isFinite(n)?String.fromCodePoint(Math.min(0x10ffff,n)):_;});
}
function strip(html='') {
...
function extractCanonical(html, base) {
  const a = (String(html).match(/<link[^>]+rel=["'][^"']*canonical[^"']*["'][^>]*href=["']([^"']+)["']/i)||[, ''])[1];
  const b = (String(html).match(/<link[^>]+href=["']([^"']+)["'][^>]*rel=["'][^"']*canonical[^"']*["']/i)||[, ''])[1];
...
function extractArticleText(html) {
  const structured = cleanContent(extractJsonLdBody(html).join(' '));
  if (structured.length >= 260) return structured;
  const semantic = [];
  for (const re of [/<article\b[^>]*>[\s\S]*?<\/article>/gi, /<main\b[^>]*>[\s\S]*?<\/main>/gi]) {
    for (const block of String(html).match(re) || []) semantic.push(strip(block));
  }
  const semanticText = cleanContent(semantic.join(' '));
  if (semanticText.length >= 300) return semanticText;
  const paragraphs = [];
  for (const m of String(html).matchAll(/<(?:h1|h2|h3|h4|p|li|blockquote)\b[^>]*>([\s\S]*?)<\/(?:h1|h2|h3|h4|p|li|blockquote)>/gi)) {
    const t = strip(m[1]); if (t.length >= 25) paragraphs.push(t);
    if (paragraphs.length >= 350) break;
  }
  const paragraphText = cleanContent(paragraphs.join(' '));
  if (paragraphText.length >= 300) return paragraphText;
  return cleanContent(strip(String(html).match(/<body\b[^>]*>[\s\S]*?<\/body>/i)?.[0] || html));
}
...
async function readBytes(res,maxBytes,deadline) {
  ...
  const chunks=[]; let total=0;
  try {
    while(total<maxBytes && left(deadline)>80){const {done,value}=await reader.read();if(done)break;if(!value)continue;const room=maxBytes-total;const c=value.byteLength>room?value.slice(0,room):value;chunks.push(c);total+=c.byteLength;}
  } finally {try{await reader.cancel();}catch{}}
  const bytes=new Uint8Array(total);let off=0;for(const c of chunks){bytes.set(c,off);off+=c.byteLength;}return bytes;
}
...
async function extractPdfText(bytes,deadline) {
  const raw=bytesLatin1(bytes); const parts=[]; const re=/<<(?:[\s\S]{0,7000}?)>>\s*stream\r?\n([\s\S]*?)\r?\nendstream/g; let m;
  ...
  // Lightweight fallback for simple/uncompressed PDFs that put readable strings
  // directly in the file instead of a stream object.
...
export function buildPreciseQueries(query,options={}) {
  const plan=options.plan||analyzeQuery(query,options); const set=new Set(); const add=x=>{x=String(x||'').replace(/\s+/g,' ').trim();if(x&&x.length>=3&&x.length<=500)set.add(x)};
  add(plan.query);
...
export function comparePageToQuery(query, page={}) {
  ...
  const titleCoverage=titleHits/n; const bodyCoverage=bodyHits/n; const phrase=phraseHit(all,plan.query);
  ...
  if(isGov(page.url)&&plan.flags.wantsOfficial)score+=7; if(isTrusted(page.url)&&(plan.flags.wantsOfficial||plan.flags.wantsAcademic))score+=5; if(ARTICLE_PATH.test(String(page.url||''))&&(plan.flags.wantsHistory||plan.flags.wantsAcademic))score+=3;
  if(plan.flags.wantsHistory && !concept.matched.includes('history') && /\bnew cars?|upcoming cars?|car prices?|buy a car|best cars?\b/i.test(all)) score-=18;
...
export function rankCandidates(candidates, queryOrPlan, options={}) {
  ...
  for(const raw of list){
    const url=normalizedUrl(raw?.url||raw?.link||raw?.sourceUrl||''); if(!url||blocked(url)||!typeFits({...raw,url},plan))continue;
    ...
    // Discovery ranking is intentionally soft. A page is not discarded merely because
    // the title/snippet does not repeat every query word.
...
export function isRealSourceContent(result){
  const c=String(result?.pageContent||result?.extractedText||'').trim(); const s=String(result?.contentStatus||'').toLowerCase(); const m=String(result?.contentMethod||'').toLowerCase();
  if(c.length<MIN_REAL_CONTENT)return false; if(['snippet','search-snippet','metadata','metadata-fallback'].includes(s)||m==='search-snippet')return false; if(blocked(result?.contentSourceUrl||result?.url))return false; return ['full','reader'].includes(s)||/direct-html|direct-pdf|jina-reader/i.test(m);
}
...
async function mapConcurrent(list,limit,worker){
  ...
  const runner=async()=>{while(true){const i=cursor++;if(i>=arr.length)return;try{out[i]=await worker(arr[i],i);}catch{out[i]=null;}}}; await Promise.all(Array.from({length:n},runner)); return out;
}
